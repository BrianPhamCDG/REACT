...
const Modal = ({ open = false, handleClose = () => {} }) => {
  ...
  return ReactDOM.createPortal(
    <div
      className={`modal fixed inset-0 z-50 p-5 flex items-center justify-center ${
        open ? "" : "opacity-0 invisible"
      }`}
    >
      <div
        className="modal-overlay absolute inset-0 bg-black bg-opacity-25"
        onClick={handleClose}
      ></div>
      <div className="modal-content relative z-10 p-10 bg-white rounded-lg w-full max-w-[482px]">
        <span
          className="modal-close absolute top-0 right-0 translate-x-2/4 -translate-y-2/4 cursor-pointer bg-white rounded-full"
          onClick={handleClose}
        >
          <svg
            width="24"
