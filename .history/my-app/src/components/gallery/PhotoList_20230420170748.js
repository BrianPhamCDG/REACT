...
const PhotoItem = ({ info: { url, id, isFavorite } }) => {
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group">
      ...
      <button className="absolute bottom-5 left-1/2 -translate-x-2/4 bg-white text-base text-black rounded-md px-6 py-2 text-sm font-medium cursor-pointer opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
        Add to card
      </button>
    </div>
  );
