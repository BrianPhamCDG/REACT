...
const PhotoItem = ({ info: { url } }) => {
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group">
      ...
      <button className="absolute bottom-5 left-1/2 -translate-x-2/4 bg-blue-500 text-base text-white rounded-md px-6 py-2 text-sm font-medium cursor-pointer opacity-0 invisible group-hover: opacity-100 visible">
        Add to card
      </button>
    </div>
  );
