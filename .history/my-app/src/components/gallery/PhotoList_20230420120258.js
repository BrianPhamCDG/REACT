...
const PhotoItem = ({ info: { url } }) => {
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group">
      ...
      <span className="absolute right-0 top-0 m-3 cursor-pointer z-10 w-8">
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
      ...
      <button className="absolute bottom-5 left-1/2 -translate-x-2/4 bg-blue-500 text-base text-white rounded-md px-6 py-2 text-sm font-medium cursor-pointer opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
        Add to card
      </button>
    </div>
  );
