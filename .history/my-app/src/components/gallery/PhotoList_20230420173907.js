...
const PhotoItem = ({ info: { url, id, isFavorite } }) => {
  console.log(url);
  const { toggleFavourite } = useGallery();
  return (
    <div className="relative h-[300px] cursor-pointer group">
      <img src={url} alt="" className="w-full h-full object-cover" />
      <span
        className="absolute right-0 top-0 m-3 cursor-pointer z-10"
      ...
      >
        ...
        >
          <path
            ...
            stroke={`${isFavorite} ? "#ff0015" : "#595959"`}
            strokeWidth="1"
            strokeLinecap="round"
      ...
      <button className="absolute bottom-5 left-1/2 -translate-x-2/4 bg-white text-black rounded-md px-6 py-2 text-sm font-medium cursor-pointer opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
        Add to card
      </button>
    </div>
  );
